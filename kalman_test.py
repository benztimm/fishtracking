...
for i in range(len(label_container_abs)):
    ...
    for j in range(TRACKER_NUM):
        ...
        if success:
            ...
            if not is_centroid_overlapping((x, y, w, h), label_container_abs[i]):
                ...
                centroid_x_tracking = int(x_new + w_new/2)
                centroid_y_tracking = int(y_new + h_new/2)
                ...
                axis_length = np.sqrt(eigenvalues) * 20  # Scale factor for visibility
                cv2.ellipse(frame, pt, (int(axis_length[0]), int(axis_length[1])), angle, 0, 360, color[j], 2)
                continue
    ...
    if position_estimates:
        ...
        # Draw the uncertainty as an ellipse
