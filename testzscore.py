...
for i in range(len(label_container_abs)):
    ...
    mean_position = np.mean(tracker_positions_array, axis=0)
    median_position = np.median(tracker_positions_array, axis=0)
    abs_deviation = np.abs(tracker_positions_array - median_position)
    mad = np.median(abs_deviation, axis=0)

    ma = stats.median_abs_deviation(tracker_positions_array)
    std = np.std(tracker_positions_array, axis=0)
    
    print(f"mean position: {mean_position}")
    print(f"median position: {median_position}")
    print(f"median absolute deviation: {mad}")
    print(f"standard deviation: {std}")
    std_dev_position = np.std(tracker_positions_array, axis=0) 
    z_scores = []
    for pos in tracker_positions_array:
        z_x = ((pos[0]-median_position))/ (1.4826 *mad[0])
        z_y = ((pos[1]-median_position))/ (1.4826 *mad[1])
        z_scores.append((z_x, z_y))
    for idx, z_score in enumerate(z_scores):
        print(f"tracker {tracker_type_name[tracker_num[idx]]} with z-score {z_score}")
    print("====================================")
